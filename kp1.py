...
class YandexDisk:
    ...
    def _create_folder(self, folder_name):
        ...
        if requests.get(url, headers=self.get.headers(), params=params).status_code != 200:
            requests.put(url, headers=self.get.headers(), params=params)
...
if __name__ == "__main__":
    ...
    tokenVK = 'TokenVK.txt'
    tokenYandex = 'TokenYD.txt'

    ya_disk = YandexDisk(get_token(tokenYandex))

    id_user = int(input('Введите идентификатор пользователя: '))
    vk_requests = VK(get_token(tokenVK), id_user)

    print(vk_requests.json)

    ya_disk.upload_file_to_disk(vk_requests.export_dict)